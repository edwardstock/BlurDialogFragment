...
public class BlurDialogEngine {
    ...
    private boolean hasNavigationBar() {
        int id = mHoldingActivity.getResources().getIdentifier("config_showNavigationBar", "bool", "android");
        boolean has = (id > 0 && mHoldingActivity.getResources().getBoolean(id)) && !ViewConfiguration.get(mHoldingActivity).hasPermanentMenuKey();
        if (!has) {
            try {
                has = hasSystemNavigationBar();
            } catch (Throwable t) {
                has = false;
            }
        }
